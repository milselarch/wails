...
import (
	...
	"github.com/milselarch/wails/v2/internal/staticanalysis"
	"github.com/milselarch/wails/v2/pkg/commands/bindings"
...
func execBuildApplication(builder Builder, options *Options) (string, error) {
	...
	if options.Platform == "windows" {
		...
		if lo.Contains(tags, nativeWebView2Loader) {
			message := "You are using the legacy native WebView2Loader. This loader will be deprecated in the near future. Please report any bugs related to the new loader: https://github.com/milselarch/wails/issues/2004"
			pterm.Warning.Println(message)
		} else {
			tags = append(tags, nativeWebView2Loader)
			message := fmt.Sprintf("Wails is now using the new Go WebView2Loader. If you encounter any issues with it, please report them to https://github.com/milselarch/wails/issues/2004. You could also use the old legacy loader with `-tags %s`, but keep in mind this will be deprecated in the near future.", strings.Join(tags, ","))
